...
void Rs232Init(void)
{
  ...
  //FOr RS-485, set the RTS pin as an output
...
void Rs232TransmitPacket(blt_int8u *data, blt_int8u len)
{
  ...
  LL_GPIO_SetOutputPin(GPIOA, LL_GPIO_PIN_1);	//since we are using RS-485, must take control of the bus first
...
blt_bool Rs232ReceivePacket(blt_int8u *data, blt_int8u *len)
{
  ...
  LL_GPIO_ResetOutputPin(GPIOA, LL_GPIO_PIN_1);	//since we are using RS-485, must now release the bus
